...
class Panels:
    def __init__(self, df_panels, df_system, df_setup, df_controller, controller_type='mppt'):
        self.df_panels = df_panels
        self.df_panels_setup = pd.DataFrame(columns=['param', 'value'])
        voc_temp = self.voc_temp()
        if controller_type == 'conventional':
            self._series_conventional(df_system, voc_temp)
            self._paralel_conventional(df_system, df_setup)
        else:
            self._series_mppt(df_controller, voc_temp)
    ...
    def _series_conventional(self, df_system, vmax):
        ...
        vmp_max_temp = vmax[1]
        serie_qty_dict = {'param': 'series', 'value': np.ceil(1.2 * v_sys / vmp_max_temp)}
    ...
    def _paralel_conventional(self, df_system, df_setup):
        ...
        i_panel = p_min / v_sys
        parallel_qty_dict = {'param': 'parallel', 'value': np.ceil(i_panel / ip_max)}
        self.df_panels_setup = self.df_panels_setup.append(parallel_qty_dict, ignore_index=True)


    def _series_mppt(self, df_controller, voc_temp):
        ...
        serie_qty_dict = {'param': 'series', 'value': [series_modules_min, series_modules_max]}
        self.df_panels_setup = self.df_panels_setup.append(serie_qty_dict, ignore_index=True)
    ...
    def _parallel_mppt(self, df_setup):
        ...
        parallel_qty_dict = {'param': 'parallel', 'value': [parallel_modules_min, parallel_modules_max]}
        self.df_panels_setup = self.df_panels_setup.append(parallel_qty_dict, ignore_index=True)
